...
export class OptimalLibraryService {
  ...
  private calculateCombinationCoverage(
    ...
    // 각 도서관마다 해당 도서관에서 빌릴 수 있는 책들 정보 생성
    const librariesWithBooks: LibraryWithBooks[] = libraries
      .map(library => {
        const libraryBooks = libraryBookMap.get(library.lib_code) || [];
        const availableBooks = libraryBooks
          .filter(isbn => requestedBooks.includes(isbn))
          .map(isbn => {
      ...
      .filter(library => library.books.length > 0); // 책이 있는 도서관만 포함

    return {
      libraries: librariesWithBooks,
      coverageCount: coveredBooks.size
    };
  }
}